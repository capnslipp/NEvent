...
* NEvent _will not work_ if placed in a *Plugins* or a *Editor* folder in your project. (It takes care of load order with its own internal folder structure.)
...
# Run the following (make sure the *Libs* directory exists beforehand):
	<pre>git submodule add git://github.com/slippyd/unity3d-NEvent.git Assets/Libs/NEvent
git submodule init
